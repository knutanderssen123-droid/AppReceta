...
class FamilyRecipesApp {
    ...
    if (!recipe) {
        this.showToast('⚠️ No se encontró la receta');
        return;
    }
    ...
    const catLabels = {
        ...
        fiesta: '🎉 Para fiestas', asiatica: '🥢 Asiática', italiana: '🇮🇹 Italiana',
        mexicana: '🌮 Mexicana', postres: '🍰 Postres', desayunos: '☀️ Desayunos',
        ensaladas: '🥗 Ensaladas', española: '🇪🇸 Española'
    };
    ...
    const ingredientsHtml = ingredients.map(function(i) {
        return '<li>' + i.charAt(0).toUpperCase() + i.slice(1) + '</li>';
    }).join('');
    ...
    const deleteBtn = recipe.customId
    ...
    const tipHtml = recipe.tip
        ? '<div class="detail-section"><h3>💡 Consejo</h3><div class="detail-tip">' + recipe.tip + '</div></div>'
        : '';

    document.getElementById('recipe-detail').innerHTML =
        '<div class="detail-hero">' +
            '<span class="detail-hero-emoji">' + (recipe.emoji || '🍽️') + '</span>' +
            '<h2>' + (recipe.name || 'Sin nombre') + '</h2>' +
        ...
        tipHtml +
        authorHtml +
        '<div style="text-align: center; margin-top: 20px;">' + deleteBtn + '</div>';
